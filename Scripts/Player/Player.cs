...
namespace PLAYERTWO.PlatformerProject
{
	...
	public class Player : Entity<Player>
	{
		...
		/// <summary>
		/// Returns the ClimbableWall instance the Player is attached to.
		/// </summary>
		public ClimbWall climbWall { get; protected set; }
		...
		/// <summary>
		/// Attaches the Player to a given ClimbableWall.
		/// </summary>
		/// <param name="other">The collider you want to evaluate as a climbable wall.</param>
		public virtual void GrabWall(Collider other)
		{
			if (!stats.current.canWallClimb || !other.CompareTag(GameTags.ClimbableWall))
				return;

			// Only attach when falling/neutral (not moving upward), hands free, and the component exists
			if (!holding && other.TryGetComponent(out ClimbWall wall))
			{
				this.climbWall = wall;
				// zero out lateral/vertical; the state sets proper velocities
				velocity = Vector3.zero;
